...
export const EmergencyIntakeModal: React.FC<EmergencyIntakeModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md overflow-y-auto">
      <div className="relative w-full max-w-4xl rounded-2xl border border-slate-800 bg-slate-900 shadow-2xl overflow-hidden glass-panel">
        {/* Modal Header */}
        <div className="flex items-center justify-between border-b border-slate-800 bg-slate-950/80 px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Multi-hospital capacity matching & automated ambulance dispatch</p>
            </div>
          </div>
          <button
        ...
        <div className="p-6">
          ...
          {step === 1 && (
            <form onSubmit={handleFindBestHospital} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-slate-800 bg-slate-950 px-3 py-2 text-xs text-white placeholder-slate-500 focus:border-teal-500 focus:outline-none"
                  />
                </div>

                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-slate-300">Age & Gender</label>
                  <div className="flex gap-2">
                    <input
                      ...
                      className="w-20 rounded-lg border border-slate-800 bg-slate-950 px-3 py-2 text-xs text-white focus:border-teal-500 focus:outline-none"
                    />
                    <select
                      value={patientGender}
                      onChange={(e) => setPatientGender(e.target.value)}
                      className="flex-1 rounded-lg border border-slate-800 bg-slate-950 px-3 py-2 text-xs text-white focus:border-teal-500 focus:outline-none"
                    >
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-slate-300">Triage Priority</label>
                  <select
                    value={priority}
                    onChange={(e) => setPriority(e.target.value as any)}
                    className="w-full rounded-lg border border-slate-800 bg-slate-950 px-3 py-2 text-xs text-white focus:border-teal-500 focus:outline-none font-bold"
                  >
              ...
              {/* Condition Summary */}
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-300">Clinical Condition Summary</label>
                <textarea
                  rows={2}
                  required
              ...
              <div className="rounded-xl border border-slate-800 bg-slate-950/40 p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                  ...
                  <div className="space-y-1">
                    <label className="text-[11px] text-slate-400">Blood Pressure</label>
                    <div className="flex items-center gap-1">
                      <input
                        type="number"
                        value={systolicBp}
                        onChange={(e) => setSystolicBp(Number(e.target.value))}
                        className="w-14 rounded-lg border border-slate-800 bg-slate-900 px-2 py-1.5 text-xs text-white font-mono"
                      />
                  ...
                  <div className="space-y-1">
                    <label className="text-[11px] text-slate-400">SpO2 (%)</label>
                    <input
                      type="number"
                      step="0.1"
                      value={spo2}
                      onChange={(e) => setSpo2(Number(e.target.value))}
                      className="w-full rounded-lg border border-slate-800 bg-slate-900 px-3 py-1.5 text-xs text-teal-400 font-mono font-bold"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[11px] text-slate-400">Resp Rate (/min)</label>
                    <input
                      type="number"
                  ...
                  <div className="space-y-1">
                    <label className="text-[11px] text-slate-400">Pickup Area</label>
                    <div className="flex items-center gap-1 text-xs text-slate-300 pt-1.5">
              ...
              <div className="rounded-xl border border-teal-500/20 bg-teal-500/5 p-4 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                  <label className="flex items-center gap-2.5 rounded-lg border border-slate-800 bg-slate-900/80 p-2.5 cursor-pointer hover:border-slate-700">
                    <input
                      type="checkbox"
                      checked={requiredIcu}
                    ...
                    />
                    <div className="text-xs font-semibold text-white">ICU Bed Required</div>
                  </label>
                  ...
                  <label className="flex items-center gap-2.5 rounded-lg border border-slate-800 bg-slate-900/80 p-2.5 cursor-pointer hover:border-slate-700">
                    <input
                      ...
                      className="rounded border-slate-700 text-teal-500 focus:ring-teal-500"
                    />
                    <div className="text-xs font-semibold text-white">Ventilator Required</div>
                  </label>
                  ...
                  <label className="flex items-center gap-2.5 rounded-lg border border-slate-800 bg-slate-900/80 p-2.5 cursor-pointer hover:border-slate-700">
                    <input
                      ...
                      className="rounded border-slate-700 text-teal-500 focus:ring-teal-500"
                    />
                    <div className="text-xs font-semibold text-white">High Flow Oxygen</div>
                  </label>
                  ...
                  <div className="space-y-1">
                    <select
                      ...
                      className="w-full rounded-lg border border-slate-800 bg-slate-900 p-2 text-xs text-white focus:border-teal-500 focus:outline-none"
                    >
          ...
          {step === 2 && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-sm font-bold text-white">Ranked Connected Hospitals ({rankings.length})</h4>
                  <p className="text-xs text-slate-400">Scored based on Distance, ETA, ICU Beds, Ventilators, Specialist & Surge Status</p>
                </div>
                <button
                  onClick={() => setStep(1)}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <span className="text-sm font-bold text-white">{r.hospital_name}</span>
                            <StatusBadge type="hospital" status={r.emergency_status} />
                          </div>
                          <p className="text-xs text-slate-400">{r.branch_name} &bull; {r.address}</p>
                        </div>

                        {/* Suitability Score & ETA */}
                        <div className="flex items-center gap-4">
                          <div className="text-right">
                            <span className="text-xs text-slate-400 block">Distance & ETA</span>
                            <span className="font-mono text-xs font-bold text-cyan-300">
                              {r.distance_km} km &bull; {r.eta_minutes} mins
                            </span>
                          </div>

                          <div className="flex flex-col items-center justify-center rounded-xl bg-slate-900 border border-slate-700/60 px-3 py-1.5 min-w-[70px]">
                            <span className="text-[10px] font-bold text-slate-400 uppercase">Score</span>
                            <span className={`text-lg font-black ${
                              r.suitability_score >= 85 ? 'text-emerald-400' :
                              r.suitability_score >= 65 ? 'text-amber-400' : 'text-rose-400'
                            }`}>
                              {r.suitability_score}<span className="text-xs font-normal text-slate-500">/100</span>
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mt-3 pt-3 border-t border-slate-800/80 grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                        ...
                        <div className={`flex items-center gap-1.5 ${r.has_specialist ? 'text-emerald-400' : 'text-slate-500'}`}>
                        ...
                        <div className="text-slate-400 flex items-center gap-1 font-mono text-[11px]">
                          ...
                          Allocates: <span className="text-white font-bold">{r.recommended_bed_code || 'Auto'}</span>
                        </div>
                      </div>
                    </div>
                  );
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-800">
                <div className="text-xs text-slate-400">
                  Selected Destination: <span className="text-white font-bold">{selectedHospital?.hospital_name}</span>
                </div>
                <div className="flex gap-3">
                  <button
                    onClick={() => setStep(1)}
                    className="rounded-xl border border-slate-800 px-4 py-2 text-xs font-semibold text-slate-400 hover:bg-slate-900"
                  >
                    Back
                  </button>
                  <button
          ...
          {step === 3 && dispatchResult && (
            <div className="py-6 text-center space-y-6">
              ...
              <div>
                <h3 className="text-xl font-bold text-white">Emergency Dispatched & Resource Reserved!</h3>
                <p className="text-xs text-slate-400 mt-1">{dispatchResult.message}</p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-2xl mx-auto text-left">
                <div className="rounded-xl border border-slate-800 bg-slate-950 p-4 space-y-1">
                  ...
                  <p className="text-sm font-bold text-white">{dispatchResult.emergency_case?.assigned_hospital_name || selectedHospital?.hospital_name}</p>
                  <p className="text-xs text-slate-400">{selectedHospital?.branch_name}</p>
                </div>
                ...
                <div className="rounded-xl border border-teal-500/30 bg-teal-500/10 p-4 space-y-1">
                  ...
                  <p className="text-sm font-bold text-white">{dispatchResult.reserved_bed?.code}</p>
                ...
                <div className="rounded-xl border border-cyan-500/30 bg-cyan-500/10 p-4 space-y-1">
                  ...
                  <p className="text-sm font-bold text-white">{dispatchResult.assigned_ambulance?.code || 'AMB-101'}</p>
