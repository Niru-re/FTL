      ...
      <div className="relative w-full max-w-lg rounded-2xl border border-slate-800 bg-slate-900 shadow-2xl p-6 glass-panel">
        <div className="flex items-center justify-between border-b border-slate-800 pb-4 mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-white">Add Clinical Documentation</h3>
              <p className="text-xs text-slate-400">{patient.full_name} ({patient.mrn}) &bull; {patient.diagnosis}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1">
            <label className="text-xs font-semibold text-slate-300">Note Classification</label>
            <select
              value={noteType}
          ...
          <div className="space-y-1">
            <label className="text-xs font-semibold text-slate-300">Clinical Assessment / Findings</label>
            <textarea
              required
              rows={4}
              value={content}
              ...
              className="w-full rounded-lg border border-slate-800 bg-slate-950 px-3 py-2 text-xs text-white placeholder-slate-500 focus:border-sky-500 focus:outline-none font-mono"
            />
          </div>

          <div className="space-y-1">
            <label className="text-xs font-semibold text-slate-300">Orders & Care Plan</label>
            <textarea
              rows={2}
              value={plan}
