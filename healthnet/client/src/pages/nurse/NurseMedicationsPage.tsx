...
export const NurseMedicationsPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Disclaimer Banner */}
      <div className="p-3.5 rounded-xl border border-slate-800 bg-slate-950/50 text-[11px] text-slate-400 flex items-start gap-2.5">
        <ShieldAlert className="h-4 w-4 text-amber-400 flex-shrink-0 mt-0.5" />
        <p>
          <strong className="text-slate-300">DEMO PROTOTYPE DATA:</strong> The pharmaceuticals, dosages, and schedules displayed are simulated for prototype demonstrations only and must not be used as clinical guidance.
        </p>
      </div>
              ...
              {loading ? (
                ...
                medications.map((med) => (
                  <tr key={med.id} className="hover:bg-slate-800/40 transition">
                    ...
                    <td className="py-3.5 text-slate-300">{med.administered_by || 'Staff Nurse'}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900 p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <button onClick={() => setActiveMed(null)} className="text-slate-400 hover:text-white">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="bg-slate-950/60 p-3 rounded-xl border border-slate-800 text-xs space-y-1">
              ...
              <p className="text-slate-400">Scheduled: {activeMed.frequency}</p>
            </div>
            ...
            <form onSubmit={handleAdministerSubmit} className="space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-950 border border-slate-800 text-xs font-bold text-white rounded-xl p-2.5 focus:outline-none focus:border-purple-500"
                >
