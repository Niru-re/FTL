      ...
      <div className="rounded-2xl border border-slate-800/80 bg-slate-900/60 p-4 backdrop-blur-md shadow-xl flex items-center justify-between">
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-slate-950/70 border border-slate-800 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-sky-500"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? (
          ...
          filteredDoctors.map((doc) => (
            <div
              key={doc.id}
              className="p-5 rounded-2xl border border-slate-800/80 bg-slate-900/60 shadow-xl space-y-4 flex flex-col justify-between hover:border-slate-700 transition"
            >
              <div>
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] font-mono text-slate-400">{doc.employee_code}</p>
                    </div>
                  </div>
                ...
                <div className="space-y-1.5 text-xs">
                  ...
                  <div className="flex items-center justify-between text-slate-300">
                    <span className="text-slate-500">Current Inpatients:</span>
                  ...
                  <div className="flex items-center justify-between text-slate-300">
                    ...
                    <span className="font-mono text-[11px] flex items-center gap-1">
                      <Phone className="h-3 w-3 text-slate-500" /> {doc.phone}
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-900 p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div>
                ...
                <p className="text-xs text-slate-400">Target Doctor: {activeDoctor.name} ({activeDoctor.specialization})</p>
              </div>
              <button onClick={() => setActiveDoctor(null)} className="text-slate-400 hover:text-white">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleRequestSubmit} className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-950 border border-slate-800 text-xs font-bold text-white rounded-xl p-2.5 focus:outline-none focus:border-sky-500"
                >
