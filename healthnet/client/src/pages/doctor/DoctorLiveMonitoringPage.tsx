          ...
          <div className="flex items-center gap-1.5 bg-slate-900 p-1 rounded-xl border border-slate-800">
            {['ALL', 'CRITICAL', 'HIGH RISK', 'STABLE'].map((r) => (
              <button
                ...
                className={`px-3 py-1 text-xs font-bold rounded-lg transition uppercase ${filterRisk === r ? 'bg-sky-600 text-white' : 'text-slate-400 hover:text-white'}`}
              >
                {r}
              </button>
            ))}
              ...
              >
                ...
                {v ? (
                  <div className="grid grid-cols-4 gap-1.5 text-center text-xs">
                    <div className={`p-1.5 rounded-lg border ${v.spo2 < 90 ? 'bg-rose-500/20 border-rose-500/40 text-rose-300 font-bold' : 'bg-slate-950/80 border-slate-800 text-slate-200'}`}>
                      <span className="text-[8px] text-slate-500 block uppercase">SpO2</span>
                      <span className="font-mono text-xs">{v.spo2}%</span>
                    </div>
                    <div className={`p-1.5 rounded-lg border ${v.heart_rate > 120 || v.heart_rate < 50 ? 'bg-rose-500/20 border-rose-500/40 text-rose-300 font-bold' : 'bg-slate-950/80 border-slate-800 text-slate-200'}`}>
                      <span className="text-[8px] text-slate-500 block uppercase">HR</span>
                      <span className="font-mono text-xs">{v.heart_rate}</span>
                    </div>
                    <div className="p-1.5 rounded-lg border bg-slate-950/80 border-slate-800 text-slate-200">
                      <span className="text-[8px] text-slate-500 block uppercase">BP</span>
                      <span className="font-mono text-[10px]">{v.systolic_bp}/{v.diastolic_bp}</span>
                    </div>
                    <div className={`p-1.5 rounded-lg border ${v.respiratory_rate >= 28 ? 'bg-rose-500/20 border-rose-500/40 text-rose-300 font-bold' : 'bg-slate-950/80 border-slate-800 text-slate-200'}`}>
                      <span className="text-[8px] text-slate-500 block uppercase">RR</span>
                      <span className="font-mono text-xs">{v.respiratory_rate}</span>
                    </div>
                  </div>
                ) : (
                  <div className="text-center py-2 text-xs text-slate-500 italic">No telemetry data</div>
                )}
