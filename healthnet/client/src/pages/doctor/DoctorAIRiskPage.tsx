...
export const DoctorAIRiskPage: React.FC = () => {
  ...
  const getVelocityBadge = (status: string) => {
    switch (status) {
      ...
      default:
        return 'text-slate-400 font-normal flex items-center gap-1';
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-slate-900/90 border border-slate-800 p-6 rounded-2xl">
        ...
        <button
          onClick={loadData}
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 text-xs font-bold uppercase tracking-wider transition active:scale-95 self-start md:self-auto"
        >
                  ...
                  >
                    <td className="py-4 px-4">
                      <div className="flex items-center gap-3">
                        <div className="h-8 w-8 rounded-full bg-slate-800 flex items-center justify-center text-teal-400 font-bold text-xs">
                          {pt.full_name.charAt(0)}
                        </div>
                        <div>
                          <div className="font-bold text-white text-sm hover:text-teal-300">{pt.full_name}</div>
                          <div className="text-[10px] text-slate-400 font-mono">{pt.mrn} • {pt.age}y {pt.gender}</div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-4 px-4">
                      ...
                      <div className="text-[10px] text-slate-400">{pt.department_name}</div>
                    </td>
                    ...
                    <td className="py-4 px-4">
                      ...
                      <span className="text-[10px] text-slate-500 font-mono uppercase">{pt.risk_velocity_status}</span>
                    </td>
