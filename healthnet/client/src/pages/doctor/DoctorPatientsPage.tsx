    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4 space-y-4 shadow-xl backdrop-blur-sm">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          ...
          <form onSubmit={handleSearchSubmit} className="md:col-span-2 relative">
            ...
            <input
              ...
              className="w-full rounded-xl bg-slate-950/80 border border-slate-800 pl-10 pr-4 py-2 text-xs text-white placeholder-slate-500 focus:border-sky-500 focus:outline-none transition"
            />
          </form>
          ...
          <div className="relative">
            <select
              ...
              className="w-full rounded-xl bg-slate-950/80 border border-slate-800 px-3.5 py-2 text-xs text-slate-200 focus:border-sky-500 focus:outline-none appearance-none font-medium cursor-pointer"
            >
        ...
        <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-800/60">
          <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mr-2">Filter Quick:</span>
      ...
      {/* Patients Table / Grid */}
      <div className="rounded-2xl border border-slate-800 bg-slate-900/60 backdrop-blur-sm shadow-xl overflow-hidden">
        <div className="p-4 border-b border-slate-800 flex items-center justify-between">
          <h2 className="text-xs font-bold uppercase tracking-wider text-slate-300">
            Assigned Inpatients ({patients.length})
          </h2>
          <span className="text-[10px] text-slate-400 font-mono">Real-time Telemetry Integrated</span>
        </div>
              ...
              >
                ...
                <div className="space-y-1.5 flex-1">
                  ...
                  <p className="text-xs font-medium text-slate-200">
                    <span className="text-slate-400">Primary Diagnosis: </span>{p.diagnosis}
                  </p>
                  ...
                  <div className="flex flex-wrap items-center gap-2 pt-1">
                    ...
                    <span className="text-[11px] text-slate-400">
                      Nurse: <strong className="text-slate-300">{p.assigned_nurse_name || 'Staff Nurse'}</strong>
                    </span>
                  </div>
                </div>
                  ...
                  {p.latest_vitals ? (
                    <>
                      <div className={`px-2.5 py-1 rounded-lg text-xs font-mono border ${
                        p.latest_vitals.spo2 < 90 ? 'bg-rose-500/20 text-rose-300 border-rose-500/30 font-bold' : 'bg-slate-950 border-slate-800 text-slate-300'
                      }`}>
                        <span className="text-[9px] text-slate-500 block">SpO2</span>
                        {p.latest_vitals.spo2}%
                      </div>
                      <div className={`px-2.5 py-1 rounded-lg text-xs font-mono border ${
                        p.latest_vitals.heart_rate > 120 || p.latest_vitals.heart_rate < 50 ? 'bg-rose-500/20 text-rose-300 border-rose-500/30 font-bold' : 'bg-slate-950 border-slate-800 text-slate-300'
                      }`}>
                        <span className="text-[9px] text-slate-500 block">HR</span>
                        {p.latest_vitals.heart_rate} bpm
                      </div>
                      <div className="px-2.5 py-1 rounded-lg text-xs font-mono bg-slate-950 border border-slate-800 text-slate-300">
                        <span className="text-[9px] text-slate-500 block">BP</span>
                        {p.latest_vitals.systolic_bp}/{p.latest_vitals.diastolic_bp}
                      </div>
                      <div className={`px-2.5 py-1 rounded-lg text-xs font-mono border ${
                        p.latest_vitals.respiratory_rate >= 28 ? 'bg-rose-500/20 text-rose-300 border-rose-500/30 font-bold' : 'bg-slate-950 border-slate-800 text-slate-300'
                      }`}>
                        <span className="text-[9px] text-slate-500 block">RR</span>
                        {p.latest_vitals.respiratory_rate}/min
                      </div>
                      <div className="px-2.5 py-1 rounded-lg text-xs font-mono bg-slate-950 border border-slate-800 text-slate-300">
                        <span className="text-[9px] text-slate-500 block">NEWS2</span>
                        <strong className={p.news2_score >= 5 ? 'text-rose-400' : 'text-slate-200'}>{p.news2_score}</strong>
                      </div>
                    </>
                  ) : (
                    <span className="text-xs text-slate-500 italic">No telemetry recorded</span>
                  )}
                </div>
